...
function mapApiOrderToOrder(data: any): Order {
  return {
    ...
    statusDates: {
      placed:
        data.statusDates?.placed ?? data.orderDate ?? '',

      processing:
        data.statusDates?.processing ?? '',

      shipped:
        data.statusDates?.shipped ?? '',

      delivered:
        data.statusDates?.delivered ?? '',
    },
    ...
    totals: {
      ...
      totalPaid: Number(
        data.totals?.totalPaid ?? data.total ?? 0
      ),
    ...
    payment: {
      cardBrandLabel:
        data.payment?.cardBrandLabel ?? '',

      last4:
        data.payment?.last4 ?? '0000',
    },
  };
}
    ...
    async function getOrder() {
      ...
      try {
        ...
        const res = await fetch(
          `/api/order/${orderId}?key=${orderKey}`
        );
